...
pub(crate) async fn present_change_mode_menu(
    ...
    match selection {
        ...
        Ok(InTheModeChoices::ClearModeSelection) => {
            let new_current_mode = NewCurrentMode::new(None);
        ...
        Ok(InTheModeChoices::SelectExistingMode(mode_node)) => {
            let new_current_mode = NewCurrentMode::new(Some(mode_node.get_surreal_id().clone()));
            send_to_data_storage_layer
                .send(DataLayerCommands::SetCurrentMode(new_current_mode))
                .await
                .unwrap();
