...
use crate::{
    ...
    surrealdb_layer::data_layer_commands::DataLayerCommands, systems::bullet_list::BulletList,
};
...
impl CreateOrUpdateChildrenItem {
    pub(crate) fn get_list(item_status: &ItemStatus<'_>) -> Vec<CreateOrUpdateChildrenItem> {
        ...
        if item_status.get_children(Filter::Active).next().is_some() {
...
pub(crate) async fn create_or_update_children(
    ...
    if !item_status.has_children(Filter::Active) {
        ...
        println!("Children:");
        for child in item_status.get_children(Filter::Active) {
            let item_lap_count = bullet_list
                .get_all_items_status()
                .iter()
                .find(|x| x.get_item() == child.get_item())
                .expect("All are here");
            println!("  {}", DisplayItemStatus::new(item_lap_count));
        }
    }
