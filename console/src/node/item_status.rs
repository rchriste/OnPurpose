...
use super::{item_node::{GrowingItemNode, ItemNode, ShrinkingItemNode}, Filter};
...
impl<'s> ItemStatus<'s> {
    ...
    pub(crate) fn get_smaller(&'s self, filter: Filter) -> Box<dyn Iterator<Item = &ShrinkingItemNode<'s>> + 's> {
    ...
    pub(crate) fn get_larger(&'s self, filter: Filter) -> impl Iterator<Item = &GrowingItemNode<'s>> {
