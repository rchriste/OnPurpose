...
impl<'s> ItemNode<'s> {
    ...
    pub(crate) fn get_smaller(&'s self, filter: Filter) -> Box<dyn Iterator<Item = &'s ShrinkingItemNode<'s>> + 's + Send> {
    ...
    pub(crate) fn get_larger(&'s self, filter: Filter) -> Box<dyn Iterator<Item = &'s GrowingItemNode<'s>> + 's + Send> {
        match filter {
    ...
    pub(crate) fn is_staging_not_set(&self) -> bool {
        ...
        if is_staging_not_set {
            //This type can be inferred from the parent so check that first
            !self.get_larger(Filter::Active).any(|x| !x.is_staging_not_set())
        } else {
            is_staging_not_set
        }
    }
