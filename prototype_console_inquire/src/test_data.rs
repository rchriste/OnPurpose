...
use crate::base_data::{
    Item, LinkageWithRecordIds, LinkageWithReferences, ReasonItem, ReviewItem, ToDo,
};
...
impl TestData {
    pub fn lookup_from_record_id<'a>(&'a self, record_id: &RecordId) -> Option<Item<'a>> {
        if let Some(found) = self.next_steps.iter().find(|x| match x.get_id() {
            Some(v) => v == record_id,
            None => false,
        }) {
            Some(Item::ToDo(found))
        } else if let Some(found) = self.review_items.iter().find(|x| match x.get_id() {
            Some(v) => v == record_id,
            None => false,
        }) {
...
#[allow(dead_code)]
pub async fn upload_test_data_to_surrealdb<T: surrealdb::Connection>(
    test_data: TestData,
    db: &Surreal<T>,
) -> TestData {
...
#[allow(dead_code)]
pub async fn upload_linkage_to_surrealdb<'a, T: surrealdb::Connection>(
    linkage: Vec<LinkageWithReferences<'a>>,
    db: &Surreal<T>,
