...
use crate::{surrealdb_layer::DataLayerCommands, base_data::NextStepItem, bullet_list::bullet_list_single_item::cover_bullet_item::cover_bullet_item};
...
async fn process_and_finish_bullet_item(next_step_item: NextStepItem, send_to_data_storage_layer: &Sender<DataLayerCommands>) {
    let user_processed_text = Editor::new("Process text").prompt().unwrap();
    send_to_data_storage_layer.send(DataLayerCommands::AddUserProcessedText(user_processed_text, next_step_item.clone())).await.unwrap();
    send_to_data_storage_layer.send(DataLayerCommands::FinishNextStepItem(next_step_item)).await.unwrap();
}
