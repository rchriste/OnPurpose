use crate::base_data::{Item, ToDo, LinkageWithReferences};
...
pub fn create_to_do_nodes<'a>(next_steps: &'a [ToDo], linkage: &'a [LinkageWithReferences<'a>]) -> Vec<ToDoNode<'a>>
{
...
pub fn create_to_do_node<'a>(to_do: &'a ToDo, linkage: &'a [LinkageWithReferences<'a>]) -> ToDoNode<'a>
{
...
mod tests {
    ...
    fn new_to_dos_are_shown_in_next_steps() {
        let next_steps = vec![ToDo { id: None, summary: "New item".into(), finished: None }];
        let linkage = vec![];
    ...
    fn finished_to_dos_are_not_shown() {
        let next_steps = vec![ToDo { id: None, summary: "Finished item".into(), finished: Some(datetime_for_testing()) }];
        let linkage = vec![];
    ...
    fn to_dos_disappear_after_they_are_covered() {
        let to_dos = vec![
            ToDo { id: None, summary: "Covered Item that should not be shown".into(), finished: None },
            ToDo { id: None, summary: "Covering Item that should be shown".into(), finished: None},
        ];
        let linkage = vec![
            LinkageWithReferences { smaller: (&to_dos[1]).into(), parent: (&to_dos[0]).into() }
        ];
    ...
    fn to_dos_return_to_next_steps_after_they_are_uncovered() {
        let next_steps = vec![
            ToDo { id: None, summary: "Covered Item to show once the covering item is finished".into(), finished: None },
            ToDo { id: None, summary: "Covering Item that is finished".into(), finished: Some(datetime_for_testing())},
        ];
        let linkage = vec![
